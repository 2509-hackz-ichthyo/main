...
func (whitespaceEncoder) Execute(cmd Command) (Result, error) {
	...
	if payload == "" {
		return Result{},
			fmt.Errorf("%w: payload must contain at least one decimal", ErrInvalidPayload)
	}
	...
	for _, token := range tokens {
		...
		if err != nil {
			return Result{},
				fmt.Errorf("%w: token %q is not an integer", ErrInvalidPayload, token)
		}

		whitespace, ok := decimalsToWhitespace[value]
		if !ok {
			return Result{},
				fmt.Errorf("%w: unsupported decimal %d", ErrInvalidPayload, value)
		}
